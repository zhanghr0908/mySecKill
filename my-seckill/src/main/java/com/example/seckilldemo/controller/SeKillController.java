...
//        }
    ...
    public RespBean getResult(TUser tUser, Long goodsId) {
        ...
        // -1：秒杀失败；0：排队中；订单id：成功；
        // 这里的轮询感觉可以去redis中去轮询查看，而不是去数据库进行轮询查看。因为之前在下订单成功之后，会将订单的用户id和商品id存放进redis里
    ...
    public RespBean doSecKill(@PathVariable String path, TUser user, Long goodsId) {
        ...
        //预减库存
        // 采用redis的decreby原子性操作来进行redis中库存数量的递减
//        Long stock = valueOperations.decrement("seckillGoods:" + goodsId);
        // 采用lua脚本实现redis中库存数量的递减
